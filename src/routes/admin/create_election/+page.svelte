<script lang="ts">
  ...
  const exisitngElectionString = $page.url.searchParams.get('election');
  const exisitingElection = JSON.parse(exisitngElectionString);

  let title = exisitingElection?.title ?? null;
  let description = exisitingElection?.description ?? '';
  let seats = exisitingElection?.seats ?? 1;
  let useQualifiedMajority = exisitingElection?.useStrict ?? false;
  let electionSystem = exisitingElection?.type ?? ElectionSystems.NORMAL;
  let electionLocation = exisitingElection
    ? exisitingElection.electionSystem ?? 'physical'
    : 'physical';
  let alternatives: FormAlternative[] = exisitingElection?.alternatives.map(
    (a: FormAlternative) => ({
      ...a,
      id: short.uuid(),
    })
